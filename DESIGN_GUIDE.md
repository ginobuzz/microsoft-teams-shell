...
- **Where it renders:** next to the agent's name in `ChatHeader` (only when the chat has sessions), and under the name/time in **only the active row** in `SessionsRail` — other sessions in the list stay tag-free so the rail doesn't turn into a wall of pills; select a session to see its tag. Both surfaces read the same per-session `tag` field.
...
- **Channel threads:** the same pill also covers an agent replying inside a channel thread, scoped to that thread rather than a session — it's the mechanism for **thread-scoped role labels**: a bot can set a short (<50 char) label describing what it's doing *in this specific thread*, distinct from any label it may have concurrently in another thread. Put `tag` (or `tagPending: true` for the pending shimmer state) directly on the reply message object in `channelPostsByContact` (`src/data/channelPosts.js`); `MessageRow` renders the pill in the message-meta row (next to sender name + the "AI generated" badge) whenever the sender is an agent and the message carries a `tag` or is `tagPending`. The label is applied **at send time, per message** — it's stamped onto each message when posted and never mutated afterward, so already-rendered messages never retroactively change; only new messages pick up whatever label is currently active for that thread.
- **Starting a thread live:** hovering any channel post shows a "Reply in thread" action (`ChatMultiple` icon) in `MessageActions`, opening `ChannelThreadRail` for that post. If Cowork hasn't joined that post's thread yet and the post has a `THREAD_SCRIPTS` entry, opening it auto-plays a fully scripted back-and-forth — nothing to type. Each script is a list of `{ from: 'me' | 'agent', text, tag?, chainOfThought?, link? }` steps played in order (typing indicator before each agent step). A step's `tag` *sets* Cowork's label for that thread — it arrives via the pending → resolved shimmer described above — and every subsequent agent message in the same script automatically **carries that label forward** (stamped at creation, no re-animation); there's no auto-expire, a label persists until explicitly replaced. Each demo thread sets its label **once**, naming the task Cowork owns there rather than restating its current phase — a thread has one label, not a running status ticker. Two demo threads (Northwind Traders • General), visited by the post-FRE demo arrow in this order: **p25-2** (API review Thursday) — Cowork sets "PR #212 review & merge" once, and a trailing confirmation message (no `tag` of its own, posted after the merge) demonstrates the carry-forward by inheriting that same label rather than switching to a new one for the merge phase; then **p25-3** (Handoff analytics) — a clarifying question before Cowork sets its own, different "watching this thread" label, showing the same agent holding two distinct labels concurrently in two threads. This all lives in `ChannelThreadRail` (`THREAD_AGENT_ID` / `THREAD_SCRIPTS` / `buildThreadTag`, guarded by `autoRunPostIds` so a script only plays once per post) — no data file changes needed.
