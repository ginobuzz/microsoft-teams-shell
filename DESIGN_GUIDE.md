...
- **Channel threads:** the same pill also covers an agent replying inside a channel thread, scoped to that reply rather than a session. Put `tag` directly on the reply message object in `channelPostsByContact` (`src/data/channelPosts.js`); `MessageRow` renders the pill in the message-meta row (next to sender name + the "AI generated" badge) whenever the sender is an agent and the message carries a `tag`. See Cowork's reply in the Launch readiness channel (`p27-4-r3`) for a live example — only that one reply shows a tag, not the whole thread, since the tag describes what the agent is doing for that specific reply.
