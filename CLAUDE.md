...
| `AgentTag` | Small purple pill (Sparkle icon + short label) that shows an agent's self-reported current task, **scoped to one session**. Two states: resolved pill, or a pending "Setting tag…" pulse shown briefly while the agent "decides" on it. | `text`, `pending = false`, `className?` | Renders `null` when there's no text and not pending. Attached via a session's `tag` field (`src/data/sessions.js`) or a `promptSuggestions.js` entry's `tag` (seeds a new session). Rendered in `ChatHeader` (next to the agent name) and `SessionsRail` (per session row) — see `DESIGN_GUIDE.md` → "Agent session tags". Tag text must stay under 50 chars. |
