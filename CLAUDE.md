...
| `AgentTag` | Small purple pill (Sparkle icon + short label) that shows an agent's self-reported current task, **scoped to one session or one channel-thread reply**. Two states: resolved pill, or a pending "Setting tag…" pulse shown briefly while the agent "decides" on it. | `text`, `pending = false`, `className?` | Renders `null` when there's no text and not pending. Attached via a session's `tag` field (`src/data/sessions.js`), a `promptSuggestions.js` entry's `tag` (seeds a new session), or directly on a channel-thread reply message (`src/data/channelPosts.js`). Rendered in `ChatHeader` (next to the agent name), `SessionsRail` (active session row only — others stay tag-free), and `MessageRow` (agent messages with a `tag`, e.g. a channel-thread reply). See `DESIGN_GUIDE.md` → "Agent session tags". Tag text must stay under 50 chars. |
| `Icon` library | Shared SVG icons exported from `components/common/Icon.jsx`: `Close`, `Plus`, `ChevronDown`, `ChevronLeft`, `ChevronRight`, `Send`, `Clock`, `Search`, `Dots`, `EmojiAdd`, `Edit`, `Check`, `CheckCircle`, `Info`, `ChatMultiple`, `Lock`, `Sparkle`. | Each takes `size`; stroked icons (`Clock`, `EmojiAdd`, `Edit`, `Check`, `CheckCircle`, `Info`, `ChatMultiple`) also take `stroke`. | When adding a new icon, start at [fluenticons.co](https://fluenticons.co/) (Fluent UI — matches Teams). Add it here instead of inlining SVG in a feature component. |
